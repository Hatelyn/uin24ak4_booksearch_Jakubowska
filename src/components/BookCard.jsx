...
export default function BookCard() {
    ...
    const getBook = async () => {
        try {
          const response = await fetch(`https://openlibrary.org/search.json?q=${slug}`) //For some reason it returs also other keys, so we need to do includdes() below still
    ...
    return (
        <section>
            ...
            <article className="singleBook"> 
                ...
                <p>Author/s: {item?.author_name ? item?.author_name.join(", ") : "Unknown author"}</p> {/* Fortsette å hadde undefinied error, så kom til at det må være noen bok som ikke har en forfatter */}
